<template>
    <div class="relative w-full bg-trso-blue transition-all duration-500 px-2 md:px-4">
        <IconsHamburger @click="menuMobileState = !menuMobileState"
                        class="block lg:hidden group absolute top-8 right-4 text-white h-8 w-8" />
        <nav>
            <NuxtLink :to="localePath({ name: 'index'})" :title="$t('pages.home.title')">
                <h1 class="text-center m-0">           
                    <img class="mx-auto lg:mx-0 w-[150px] h-[54px] my-4 lg:my-0" 
                        width="200" 
                        height="72" 
        ...
        <button class="block sm:flex items-center gap-x-4 absolute top-6 sm:top-9 lg:top-1/2 lg:-translate-y-1/2 left-4 lg:right-4 lg:left-auto"
